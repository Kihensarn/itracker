...
from ITrackerModel_xgaze import ITrackerModel
from ITrackData_Xgaze import ITrackDataXgaze

parse = argparse.ArgumentParser(description='iTracker-xgaze-trainer')
parse.add_argument('--data_path', default='/home/data/wjc_data/xgaze_224_prepare_two',
...
workers = 16
epochs = 25
batch_size = torch.cuda.device_count() * 16
...
def main():
    ...
    scheduler = StepLR(optimizer, step_size=4, gamma=0.1, last_epoch=-1)
    ...
    if do_train:
        ...
        logger.add(Path(data_path).joinpath('model_dir/model_val.log'),
                   filter=lambda x: '(validate)' in x['message'])
        logger.add(Path(data_path).joinpath('model_dir/model_train.log'),
        ...
        for epoch in range(load_epoch+1, epochs):
            ...
            train(train_loader, model, optimizer, scheduler, epoch, vis)

            current_predict = validate(val_loader, model, epoch, vis)
...
def train(train_loader, model, optimizer, scheduler, epoch, vis):
    global count_train
    ...
    for i, (imFace, imEyeL, imEyeR, gaze_direction) in enumerate(train_loader):
        ...
        loss = F.smooth_l1_loss(output, gaze_direction)
...
def validate(val_loader, model, epoch, vis):
    global count_test
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    ...
    for i, (imFace, imEyeL, imEyeR, gaze) in enumerate(val_loader):
        ...
        loss = F.smooth_l1_loss(output, gaze)
